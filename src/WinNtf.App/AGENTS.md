...
- `PopupPresenter` 实现 `INotificationSink`，默认 `maxVisible=3`；通过 `NotificationSlotQueue` 取槽，满了**静默丢弃**（返回 `CompletedTask`）。show 必须经 `Application.Current.Dispatcher.InvokeAsync` 封送到 UI 线程。
- popup 关闭时通过回调 `_slots.Release(slot)` 归还槽位。
