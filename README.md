...
- 默认写入当前用户开机/登录自启动项，配置在 `%APPDATA%/win-ntf/config.json`；`maxVisible` 默认 `10`。
...
配置文件位于 `%APPDATA%/win-ntf/config.json`：
...
- `port`: loopback HTTP 端口，默认 `9876`。
- `startOnLogin`: 是否写入当前用户登录自启动，默认 `true`。
- `defaultPosition`: 默认 popup 位置，`TopRight | BottomRight | Center`。
- `maxVisible`: 同屏最多可见 popup 数量，默认 `10`；满载时新通知会顶掉最早的 popup，而不是丢弃新通知。
- 仍在显示的完全相同通知会被去重：已有 popup 重置倒计时并移到栈顶，不会重复开新窗口。
